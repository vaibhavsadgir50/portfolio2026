...
const STAR_ICON = (
  <svg className="icon" viewBox="0 0 24 26" aria-hidden>
...
export default function Button({ children, href, onClick, type = 'button', className = '' }: ButtonProps) {
  ...
  useEffect(() => {
    ...
    const stars = gsap.to(el, {
      repeat: -1,
      repeatDelay: 0.75,
      paused: true,
      keyframes: [
        {
          '--generate-button-star-2-scale': 0.5,
          '--generate-button-star-2-opacity': 0.25,
        ...
        },
        {
          ...
          '--generate-button-star-1-opacity': 0.5,
          '--generate-button-star-2-scale': 0.5,
    ...
    const onEnter = () => stars.restart().play();
    const onLeave = () => {
      gsap.to(el, {
        '--generate-button-star-1-opacity': 0.25,
        '--generate-button-star-1-scale': 1,
  ...
  const cls = `generate-button ${className}`.trim();

  if (href) {
    return (
      <a ref={ref as React.RefObject<HTMLAnchorElement>} href={href} className={cls} target="_blank" rel="noopener noreferrer">
        {content}
      </a>
    );
  }
  ...
  return (
    <button ref={ref as React.RefObject<HTMLButtonElement>} type={type} className={cls} onClick={onClick}>
      {content}
    </button>
  );
}
